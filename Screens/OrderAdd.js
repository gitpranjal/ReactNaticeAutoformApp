...
class OrderAdd extends React.Component {
  ...
  submit = () => {
    ...
    else if(this.state.SelectedBrands===null){
        checkflag=1
        errmsg="Select Brand"
    }
    ...
    if (checkflag === 0) {
      ...
      body = JSON.stringify({
        ...
        orderParams: {
          ...
          brandID:this.state.SelectedBrands.id,
...
<SearchableDropdown
          //On text change listner on the searchable input
          onTextChange={(text) => console.log(text)}
          onItemSelect={item => {this.savestyle(item)
          }}
          //onItemSelect called after the selection from the dropdown
          containerStyle={{ padding: 8 ,width:Dimensions.get("window").width / 1.1 ,
           borderWidth:3,
...
OrderAdd.navigationOptions = (navData) => {
  return {
