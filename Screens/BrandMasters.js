...
import BrandSuperItem from "../components/BrandSuperItem";
import { CheckBox } from "react-native-elements";
import { SearchBar } from "react-native-elements";

class BrandMasters extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  fetchdata = () => {
    ...
    fetch(
      "https://qualitylite.bluekaktus.com/api/bkQuality/masters/getBrandDetails",
      {
        method: "POST",
        body: JSON.stringify({
          basicparams: {
  ...
  searchFilterFunction = (text) => {
    ...
    if (!text || text == "") {
      ...
      const newData = this.state.fullData.filter((item) => {
        const itemData = `${item.brandName.toUpperCase()}`;
  ...
  renderGridItem = (itemData) => {
    ...
    return (
      <BrandSuperItem
        name={itemData.item.brandName}
        id={itemData.item.brandID}
...
BrandMasters.navigationOptions = (navData) => {
  return {
    headerTitle: "Brands",
    headerStyle: {
      backgroundColor: Colors.primaryColor,
