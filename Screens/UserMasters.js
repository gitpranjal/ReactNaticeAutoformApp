...
class UserMasters extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  searchFilterFunction = (text) => {
    ...
    if (!text || text == "") {
      ...
      const newData = this.state.fullData.filter((item) => {
        const itemData = `${item.loginID.toUpperCase()}`+`${item.userFName.toUpperCase()}`+`${item.userLName.toUpperCase()}`;
  ...
  renderGridItem = (itemData) => {
    ...
    return (
      <UserSuperItem
        username={itemData.item.loginID}
        fn={itemData.item.userFName}
        ln={itemData.item.userLName}
        phNo={itemData.item.phoneNo}
        roleDesc={itemData.item.userRoleDesc}
        onSelect={() => {
...
UserMasters.navigationOptions = (navData) => {
  return {
    headerTitle: "Users",
    headerStyle: {
      backgroundColor: Colors.primaryColor,
