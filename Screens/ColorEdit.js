...
class ColorEdit extends React.Component {
  ...
  submit = () => {
    ...
    if (this.state.name === "") {
      checkflag = 1;
      this.setState({
        namehint: "Please Enter Color",
        namehintColor: "#fb7a7480",
      });
    }
    if (checkflag === 0) {
      fetch(
        "https://qualitylite.bluekaktus.com/api/bkQuality/masters/PostColorDetails",
        {
          method: "POST",
          body: JSON.stringify({
            basicparams: {
...
ColorEdit.navigationOptions = (navData) => {
  return {
    headerTitle: "Edit Color",
    headerStyle: {
      backgroundColor: Colors.primaryColor,
    },
