...
import ColorSuperItem from "../components/ColorSuperItem";
import { CheckBox } from "react-native-elements";
import { SearchBar } from "react-native-elements";

class ColorMasters extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  fetchdata = () => {
    ...
    fetch(
      "https://qualitylite.bluekaktus.com/api/bkQuality/masters/getColorDetails",
      {
        method: "POST",
        body: JSON.stringify({
          basicparams: {
  ...
  searchFilterFunction = (text) => {
    ...
    if (!text || text == "") {
      ...
      const newData = this.state.fullData.filter((item) => {
        const itemData = `${item.colorName.toUpperCase()}`;
  ...
  renderGridItem = (itemData) => {
    ...
    return (
      <ColorSuperItem
        name={itemData.item.colorName}
        id={itemData.item.colorID}
...
ColorMasters.navigationOptions = (navData) => {
  return {
    headerTitle: "Colors",
    headerStyle: {
      backgroundColor: Colors.primaryColor,
