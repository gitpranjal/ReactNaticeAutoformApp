...
import ProcessSuperItem from "../components/ProcessSuperItem";
import { CheckBox } from "react-native-elements";
import { SearchBar } from "react-native-elements";

class ProcessMaster extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  searchFilterFunction = (text) => {
    ...
    if (!text || text == "") {
      ...
      const newData = this.state.fullData.filter((item) => {
        const itemData = `${item.processName.toUpperCase()}`;
  ...
  renderGridItem = (itemData) => {
    ...
    return (
      <ProcessSuperItem
        name={itemData.item.processName}
        id={itemData.item.processID}
        status={itemData.item.status}
        color={colorcheck(itemData.item.status)}
        onSelect={() => {
          this.props.navigation.navigate({
            routeName: "ProcessEdit",
            params: {
              name: itemData.item.processName,
...
ProcessMaster.navigationOptions = (navData) => {
  return {
